...
async function request(path:string,init?:RequestInit){const response=await fetch(configuredBase()+path,{...init,headers:{'Content-Type':'application/json',...(session.token?{Authorization:`Bearer ${session.token}`}:{}) ,...init?.headers}});if(!response.ok){const raw=await response.text();let reason="";try{const body=JSON.parse(raw);reason=body.error||body.detail||body.title||""}catch{reason=raw}if(!reason)reason=response.status===403?"You do not have permission to perform this action with the current account.":response.status===401?(path.startsWith('/auth/')?"Invalid staff or PIN.":"Your session expired. Sign in again."):`Request failed (${response.status}).`;throw new Error(reason)}const result=response.status===204?null:await response.json();const method=(init?.method||'GET').toUpperCase();if(method!=='GET'&&!path.startsWith('/auth/'))dispatchEvent(new CustomEvent('thebarcode:data-changed',{detail:{path,method,at:Date.now()}}));return result;}
