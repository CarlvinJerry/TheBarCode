...
function downloadPdfReport({from,to,summary,detail,accounting,accountingTrial}:{from:string;to:string;summary:any;detail:any;accounting?:any;accountingTrial?:any[]}){
  ...
  const revenue=Number(summary.revenue||0),cost=Number(summary.cost||0),expenses=Number(summary.expenses||0),gross=revenue-cost,net=gross-expenses,collectionRate=Number(summary.collectionRate||0),cashCollected=Number(summary.cashCollected||0),fmt=(n:number)=>{const value=Number(n||0);if(Math.abs(value)<0.005)return "—";const text=`${org.currency} ${Math.abs(value).toLocaleString(undefined,{maximumFractionDigits:0})}`;return value<0?`(${text})`:text};
  ...
  let nextY=25;
  if(accounting){
    doc.setFontSize(10);doc.setFont("helvetica","bold");doc.text("Accounting snapshot",14,nextY-7);doc.setFont("helvetica","normal");
    autoTable(doc,{startY:nextY,head:[["Statement line","Amount"]],body:[["Revenue",fmt(Number(accounting.revenue||0))],["Cost of goods sold",fmt(-Number(accounting.cost||0))],["Gross profit",fmt(Number(accounting.grossProfit||0))],["Approved expenses",fmt(-Number(accounting.expenses||0))],["Net profit",fmt(Number(accounting.netProfit||0))],["Receivables at period end",fmt(Number(accounting.receivables||0))],["Payables at period end",fmt(Number(accounting.payables||0))]],theme:"grid",headStyles:{fillColor:green},styles:{fontSize:8}});
    nextY=(doc as any).lastAutoTable.finalY+7;
    if(accountingTrial?.length){autoTable(doc,{startY:nextY,head:[["Trial balance account","Debit","Credit"]],body:accountingTrial.slice(0,30).map((x:any)=>[`${x.code} · ${x.name}`,fmt(Number(x.debit||0)),fmt(Number(x.credit||0))]),theme:"striped",headStyles:{fillColor:green},styles:{fontSize:7}});nextY=(doc as any).lastAutoTable.finalY+7;}
  }
  autoTable(doc,{startY:nextY,head:[["Payment method","Collected"]],body:(detail.paymentMix||[]).map((x:any)=>[x.name,fmt(Number(x.amount))]),theme:"grid",headStyles:{fillColor:green},styles:{fontSize:8}});
