...
function Bills({products,user,notify}:{products:Product[];user:{id:string;name:string;role:string};notify:(x:string)=>void}){
  ...
  const post=async(kind:"Paid"|"Credit",customerId?:string)=>{if(kind==="Credit"&&!customerId&&!selected.customerId){setCreditOpen(true);return}try{const revised=await updateBill(selected.id,{...revisionPayload(customerId),reason:"Final bill revision before posting"});const posted=await postBill(revised.id,{status:kind,method:kind==="Paid"?payment.method:"Credit",amountPaid:kind==="Paid"?revised.total:0,dueAt:kind==="Credit"?new Date(Date.now()+7*86400000).toISOString():undefined,notes:"Posted from bill workspace",deviceId:localStorage.getItem("device_id")});setSelected(undefined);setCreditOpen(false);notify(`${kind} bill posted via ${kind==="Paid"?payment.method:"customer credit"}; revision ${posted.revision}, stock, dashboards and reports updated`);dispatchEvent(new Event("dukora:attention"));await load()}catch(e){notify(e instanceof Error?e.message:"Bill could not be posted")}};
...
function Inventory({
  ...
  async function save(e: FormEvent) {
    ...
    await syncOutbox().catch(() => 0);await bootstrap().catch(()=>0);dispatchEvent(new Event("dukora:attention"));
    setOpen(false);
