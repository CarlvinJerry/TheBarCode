...
public static class ExpenseEndpoints
{
    public static void MapExpenseApi(this WebApplication app)
    {
        ...
        {
            if(string.IsNullOrWhiteSpace(r.Category)||string.IsNullOrWhiteSpace(r.Description)||r.Amount<=0||r.InitiallyPaid<0||r.InitiallyPaid>r.Amount)return Results.BadRequest(new{error="Category, description and a valid amount/payment are required"});
        ...
        }).RequireAuthorization(p=>p.RequirePermission("expenses"));
        api.MapPost("/{id:guid}/approve",async(Guid id,AppDbContext db,ClaimsPrincipal principal)=>{var expense=await db.Expenses.SingleOrDefaultAsync(x=>x.Id==id&&x.IsDemo==Security.IsDemo(principal)&&x.Active);if(expense is null)return Results.NotFound();expense.Status="Approved";expense.UpdatedAt=DateTimeOffset.UtcNow;db.AuditEvents.Add(Audit(principal,"Approved",expense,"Expense approved for reporting and payment","expense-approval"));await db.SaveChangesAsync();return Results.Ok(expense);}).RequireAuthorization(p=>p.RequirePermission("approvals"));
        api.MapPost("/{id:guid}/payments",async(Guid id,ExpensePaymentRequest r,AppDbContext db,ClaimsPrincipal principal)=>
        {
            if(r.Amount<=0||string.IsNullOrWhiteSpace(r.Method))return Results.BadRequest(new{error="A positive payment amount and method are required"});var demo=Security.IsDemo(principal);var expense=await db.Expenses.SingleOrDefaultAsync(x=>x.Id==id&&x.IsDemo==demo&&x.Active);if(expense is null)return Results.NotFound();if(expense.Status!="Approved")return Results.Conflict(new{error="Approve this expense before recording payment"});var paid=await db.ExpensePayments.Where(x=>x.ExpenseId==id).SumAsync(x=>(decimal?)x.Amount)??0m;var balance=Math.Max(0,expense.Amount-paid);if(r.Amount>balance)return Results.BadRequest(new{error=$"Payment exceeds the outstanding balance of {balance}"});var payment=new ExpensePayment{ExpenseId=expense.Id,Amount=Math.Round(r.Amount,2),Method=r.Method.Trim(),Reference=r.Reference?.Trim(),Notes=r.Notes?.Trim(),StaffId=StaffId(principal),PaidAt=DateTimeOffset.UtcNow};db.ExpensePayments.Add(payment);expense.PaidAmount=paid+payment.Amount;expense.Method=payment.Method;expense.UpdatedAt=DateTimeOffset.UtcNow;var remaining=Math.Max(0,expense.Amount-expense.PaidAmount);db.AuditEvents.Add(Audit(principal,"Paid",expense,$"Expense payment {payment.Amount} · balance {remaining}",r.DeviceId));await db.SaveChangesAsync();return Results.Ok(new{expense.Id,expense.PaidAmount,balance=remaining,payment});
        ...
        {
            if(string.IsNullOrWhiteSpace(r.Reason)||string.IsNullOrWhiteSpace(r.Category)||string.IsNullOrWhiteSpace(r.Description)||string.IsNullOrWhiteSpace(r.Method)||r.Amount<=0)return Results.BadRequest(new{error="Date, category, description, positive amount, method and change reason are required"});var expense=await db.Expenses.SingleOrDefaultAsync(x=>x.Id==id&&x.IsDemo==Security.IsDemo(principal));if(expense is null)return Results.NotFound();if(r.Amount<expense.PaidAmount)return Results.Conflict(new{error=$"Amount cannot be below the already paid total of {expense.PaidAmount}"});var before=$"{expense.Date} · {expense.Category} · {expense.Amount} · {expense.Description}";expense.Date=r.Date;expense.Category=r.Category.Trim();expense.Description=r.Description.Trim();expense.Amount=r.Amount;expense.Method=r.Method.Trim();expense.Payee=r.Payee?.Trim();expense.Reference=r.Reference?.Trim();expense.DueDate=r.DueDate;expense.TaxAmount=Math.Max(0,r.TaxAmount);expense.Notes=r.Notes?.Trim();expense.Recurring=r.Recurring;expense.Active=r.Active;expense.Status=r.Active?(expense.Status=="Archived"?"Approved":expense.Status):"Archived";expense.UpdatedAt=DateTimeOffset.UtcNow;db.AuditEvents.Add(Audit(principal,"Updated",expense,$"{r.Reason.Trim()} · from {before}","expense-editor"));await db.SaveChangesAsync();return Results.Ok(expense);
        }).RequireAuthorization(p=>p.RequirePermission("approvals"));
    }
