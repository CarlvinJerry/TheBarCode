...
{
 public static async Task Initialize(AppDbContext db,string bootstrapPin,bool resetAdminPin=false)
 {
  await db.Database.EnsureCreatedAsync();
  if(db.Database.IsNpgsql()) foreach(var statement in new[]{"ALTER TABLE staff ADD COLUMN IF NOT EXISTS is_demo boolean NOT NULL DEFAULT false","ALTER TABLE products ADD COLUMN IF NOT EXISTS is_demo boolean NOT NULL DEFAULT false","ALTER TABLE customers ADD COLUMN IF NOT EXISTS is_demo boolean NOT NULL DEFAULT false","ALTER TABLE expenses ADD COLUMN IF NOT EXISTS is_demo boolean NOT NULL DEFAULT false",
  ...
  {
   var admin=new StaffUser{Name="Admin",Role="Owner",PinHash=Security.HashPin(bootstrapPin)};
   db.Staff.Add(admin);
   db.AuditEvents.Add(new AuditEvent{StaffId=admin.Id,Actor=admin.Name,Action="Seeded",EntityType="System",Details="Created owner account",DeviceId="server"});
   await db.SaveChangesAsync();
  }
  else if(resetAdminPin)
  {
   // Recovery is deliberately opt-in. It never changes an existing PIN on normal startup.
   var admin=await db.Staff.FirstOrDefaultAsync(x=>x.Name=="Admin"&&x.Role=="Owner"&&!x.IsDemo);
   if(admin is not null&&!Security.VerifyPin(bootstrapPin,admin.PinHash))
   {
    admin.PinHash=Security.HashPin(bootstrapPin);
    admin.UpdatedAt=DateTimeOffset.UtcNow;
    db.AuditEvents.Add(new AuditEvent{StaffId=admin.Id,Actor="System",Action="Updated",EntityType="Staff",EntityId=admin.Id.ToString(),Details="Owner PIN repaired by explicit bootstrap recovery",DeviceId="server"});
    await db.SaveChangesAsync();
   }
  }
  if(!await db.Organizations.AnyAsync()) db.Organizations.Add(new OrganizationProfile{Name="The BarCode",LegalName="",IndustryProfile="BarCafe",Currency="KES",Tagline="Smart business operations"});
  if(!await db.Branches.AnyAsync()) db.Branches.Add(new Branch{Name="Main branch",Code="MAIN"});
  if(!await db.ReceiptConfigurations.AnyAsync()) db.ReceiptConfigurations.Add(new ReceiptConfiguration());
  await db.SaveChangesAsync();
 }
}
