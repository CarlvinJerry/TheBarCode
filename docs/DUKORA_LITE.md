# TheBarcode Lite for Windows

TheBarcode Lite is the low-resource, offline-first Windows edition. It uses the same web interface, API, business rules, reports, Smart Insights and receipt-printing integration as TheBarcode Server.

## What changes

- SQLite replaces the separately installed PostgreSQL service.
- `TheBarcode.Desktop.exe` starts the local API and print bridge only while TheBarcode is open.
- Microsoft WebView2 displays the existing React UI in a branded desktop window.
- Data is stored per Windows user under `%LOCALAPPDATA%\Beyond Raw Data\TheBarcode`.
- The owner chooses a private PIN on first launch; it is protected with Windows DPAPI.

## Installation

Installer: `installer\output\TheBarcode-Setup-<version>-x64.exe`

Upgrades replace application binaries only. Institution data and configuration remain under `%LOCALAPPDATA%\Beyond Raw Data\TheBarcode`, outside the installation directory, and are never deleted by setup or uninstall. TheBarcode migrates the former `%LOCALAPPDATA%\Beyond Raw Data\Dukora Lite` folder and database on first launch, creates a daily pre-start database backup and retains the latest 14 backups. Data removal occurs only through an explicit authorized action inside TheBarcode.
...
1. Run `TheBarcode-Setup-<version>-x64.exe` and approve Windows setup.
2. Leave **Create a TheBarcode desktop shortcut** selected if desired.
...
Back up `%LOCALAPPDATA%\Beyond Raw Data\TheBarcode\thebarcode.db` while TheBarcode is closed. A future migration utility will move the same domain records to the PostgreSQL Server or hosted Cloud edition. Do not copy a live SQLite WAL database while TheBarcode is open.
