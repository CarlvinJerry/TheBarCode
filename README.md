...
Current version: **1.9.1** (see `VERSION`). Native installers must be rebuilt after every releasable change so the installer always contains the current UI, API, database initialization and print bridge. Version 1.9.1 is intentionally awaiting local acceptance testing before its installer is generated.
